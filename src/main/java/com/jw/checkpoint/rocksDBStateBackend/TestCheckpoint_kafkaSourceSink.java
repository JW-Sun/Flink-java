...
import com.jw.checkpoint.utils.KafkaUtil;
...
public class TestCheckpoint_kafkaSourceSink {
    public static void main(String[] args) throws Exception {
        ...
        StateBackend stateBackend = new RocksDBStateBackend("hdfs://192.168.159.102:9000/flink-checkpoint/RocksDBStateBackend", true);
        ...
        DataStreamSource<String> source = env.addSource(new KafkaUtil().getFlinkKafkaConsumer("test02"));
        ...
        prop.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, "192.168.159.102:9092");

        sum.map(new MapFunction<Wc, String>() {
                    @Override
                    public String map(Wc value) throws Exception {
                        return JSON.toJSONString(value);
                    }
                })
                //.addSink(KafkaUtil.getFlinkKafkaProducer("test"));
                .addSink(new FlinkKafkaProducer011<String>(
                        "192.168.159.102:9092",
                        "test",
